...
function update_trimester($trimester_id, $conn) {
    ...
    while ($current=mysqli_fetch_assoc($all_course)) {
        ...
        if ($current['total_marks'] != 0) {
            ...
            $ex_marks = 0; // or some other default value
        }
...
function delete_course($course_id, $conn) {
    ...
    // removing all participants enrolled in the course
    $conn->query("DELETE FROM participants WHERE course_id=$course_id");
